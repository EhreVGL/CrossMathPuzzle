...
public class CreateBoard1 : MonoBehaviour
{
    // Tahta boyutu değerleri
    public const int BOARD_SIZE_X = 7;
    public const int BOARD_SIZE_Y = 7;
    ...
    public const int PIECE_COUNT = 9;
    ...
    private string[,] OPERATION_VALUES = new string[PIECE_COUNT + 1, 3];

    // Tahta array
    private string[,] board = new string[BOARD_SIZE_X, BOARD_SIZE_Y];
    ...
    private bool PlaceNumber(int x, int y)
    {
        ...
        OPERATION_VALUES[OPERATION_COUNT, 0] = num1.ToString();
        OPERATION_VALUES[OPERATION_COUNT, 1] = num2.ToString();
        OPERATION_VALUES[OPERATION_COUNT, 2] = result.ToString();
        OPERATION_COUNT++;
    ...
    private void ResetOperationValues()
    {
        ...
        for (int i = 0; i < OPERATION_VALUES.GetLength(0); i++)
        {
            for (int j = 0; j < OPERATION_VALUES.GetLength(1); j++)
            {
                OPERATION_VALUES[i, j] = ""; // Her hücreyi boş (""), yani sıfırla
            }
        }
    }
    ...
    private void StartFunction()
    {
        ...
        for (int i = 0; i < PIECE_COUNT; i++)
        {
            ...
            for (int j = 0; j < 3; j++)
            {
                if (OPERATION_VALUES[i, j] == "0") // string "0" ile karşılaştırıyoruz
                {
                    zeroCounter += 1;
                }
            }
